...
app.controller("verEmpCtrl", function ($scope, $http, $location) {
  ...
  $http
    .get("http://143.110.233.84/empleado/getEmpleados", {})
...
app.controller("verCteCtrl", function ($scope, $http, $location) {
  ...
  $scope.detalle = true;
  $http
    .get("http://143.110.233.84/user/filtroFecha", {})
    .then(function (respuesta) {
  ...
  $http
    .get("http://143.110.233.84/user/clientes", {})
    .then(function (respuesta) {
...
app.controller("clientesCtrl", function ($scope, $http, $location) {
  ...
  $scope.Enviar = function () {
    ...
    $http
      .post("http://143.110.233.84/user/add-user", $scope.valores)
      .then(function (respuesta) {
...
app.controller("expedienteCtrl", function ($scope, $http, $location) {
  ...
  $scope.EnviarExp = function () {
    ...
    $http
      .post("http://143.110.233.84/user/add-expediente", $scope.datos)
      .then(function (respuesta) {
        console.log(respuesta.data);
        if (respuesta.data.status == 1) {
          $scope.res = true;
...
app.controller("empleadosCtrl", function ($scope, $http, $location) {
  ...
  $scope.Enviar = function () {
    ...
    $http
      .post(
        "http://143.110.233.84/empleado/add-empleado",
        //$http.post("http://143.110.233.84/empleado/add-empleado",
        $scope.valores
      )
      .then(function (respuesta) {
        console.log(respuesta.data);
...
app.controller(
  ...
  function ($scope, $http, $location, $routeParams) {
    ...
    cambiarDatos = function (correo, telefono, calle, numero, cp) {
      ...
      $http
        .post("http://143.110.233.84/user/modificarDatosCliente/", data)
        .then(function (respuesta) {
          console.log(respuesta.data);
          $scope.exitoD = true;
        });
    ...
    $scope.actualizarExpediente = function () {
      ...
      let nombreMedicamentoDC = document.getElementById(
        "nombreMedicamentoDC"
      ).value;
      ...
      let formaAlimentacion =
        document.getElementById("formaAlimentacion").value;
      ...
      let eventoRelacionado =
        document.getElementById("eventoRelacionado").value;
    ...
    cambiarExpediente = function (
      ...
      $http
        .post("http://143.110.233.84/user/modificarExpedienteCliente/", data)
        .then(function (respuesta) {
          console.log(respuesta.data);
          $scope.exitoE = true;
        });
    ...
    $http.get(ruta, {}).then(function (respuesta) {
      console.log(respuesta.data);
...
app.controller(
  ...
  function ($scope, $http, $location, $routeParams) {
    ...
    $scope.cte = $routeParams.idcte;
    $scope.input = false;
    var ruta = "http://143.110.233.84/consultas/consulta_cliente/" + $scope.cte;
    $http.get(ruta, {}).then(function (respuesta) {
    ...
    var ruta2 = "http://143.110.233.84/user/cliente/" + $scope.cte;

    $http.get(ruta2, {}).then(function (respuesta) {
      $scope.nombre =
        respuesta.data.usuario[0].nombre +
        " " +
        respuesta.data.usuario[0].apellidoPaterno +
        " " +
        respuesta.data.usuario[0].apellidoMaterno;
    });
    ...
    $scope.Enviar = function (data) {
      $scope.exito = false;
      $scope.input = false;
      const ruta = "http://143.110.233.84/consultas/modificarConsulta";
...
app.controller(
  ...
  function ($scope, $http, $location, $routeParams) {
    ...
    //var ruta = "http://143.110.233.84/empleado/" + $scope.emp;
    var ruta = "http://143.110.233.84/empleado/" + $scope.emp;
    ...
    cambiarDatos = function (
      ...
      $http
        .post("http://143.110.233.84/empleado/modificarDatosEmpleado/", data)
        .then(function (respuesta) {
          console.log(respuesta.data);
          $scope.exito = true;
        });
...
app.controller("citasCtrl", function ($scope, $http, $location) {
  ...
  $scope.registrarCita = function (data) {
    ...
    $scope.datosConsultaIn.hora =
      document.getElementById("horaIn").value +
      " - " +
      document.getElementById("horaFin").value;
    console.log($scope.datosConsultaIn);
    $http
      .post("http://143.110.233.84/citas/addCita", $scope.datosConsultaIn)
      .then(function (respuesta) {
  ...
  $http.get("http://143.110.233.84/user/clientes").then(function (respuesta) {
...
app.controller(
  ...
  function ($scope, $http, $location, $routeParams) {
    ...
    let ruta = "http://143.110.233.84/empleado/" + idemp;
    $scope.empleado;
    ...
    function buscarCitasPorClienteYFecha(fecha, idEmpleado) {
      ...
      console.log(datos);
      let ruta = "http://143.110.233.84/citas/fecha-idEmpleado";
      $http.post(ruta, datos).then(function (respuesta) {
        ...
        if ($scope.citasEmpleado.length > 0) {
          ...
          for (let i = 0; i < $scope.citasEmpleado.length; i++) {
            $scope.citasEmpleado[i].fecha =
              $scope.citasEmpleado[i].fecha.split("T")[0];
          }
        } else {
          $scope.tabla = false;
    ...
    $scope.enviarWA = function (data) {
      ...
      $http
        .post("http://143.110.233.84/citas/enviarRecordatorio", data.x)
        .then(function (respuesta) {
          console.log("res:", respuesta.data.response[0].telefono, $scope);
          if (respuesta.data.status == 1) {
            $scope.res = true;
            var mensaje =
              "Se le recuerda que el día " +
              data.x.fecha +
              " en el horario de " +
              data.x.hora +
              " tiene una cita en el consultorio de NaturalInn. ¡Que tenga un buen día!";
            open(
              "https://api.whatsapp.com/send?phone=521" +
                respuesta.data.response[0].telefono +
                "&text=" +
                mensaje +
                "&source=&data="
            );
...
app.controller("consultasCtrl", function ($scope, $http, $location) {
  ...
  $scope.cambio = function (event) {
    if (event.split("|")[0] !== undefined) {
      const id = event.split("|")[0];
      var ruta = "http://143.110.233.84/consultas/consulta_cliente/" + id;
      $http.get(ruta, {}).then(function (respuesta) {
        if (respuesta.data.consultas != undefined) {
          $scope.consultas = respuesta.data.consultas;
          console.log(respuesta.data);
          for (var i = 0; i < respuesta.data.consultas.length; i++) {
            var fecha = respuesta.data.consultas[0].fecha.split("T");
  ...
  $scope.RegistrarConsulta = function () {
    ...
    $http
      .post("http://143.110.233.84/consultas/registrarConsulta", $scope.valores)
      .then(function (respuesta) {
        console.log(respuesta.data);
        if (respuesta.data.status == 1) {
          $scope.res = true;
...
app.controller("verConsultaCtrl", function ($scope, $http, $location) {
  ...
  $http
    .get("http://143.110.233.84/consultas/clientes_con_consultas", {})
    .then(function (respuesta) {
...
app.controller("masajesCtrl", function ($scope, $http, $location) {
  ...
  $scope.buscarFecha = function () {
    ...
    console.log(fecha);
    $http
      .get("http://143.110.233.84/masajes/citasOcupadas/" + fecha)
      .then(function (respuesta) {
        console.log(respuesta.data);
  ...
  $scope.Enviar = function (data) {
    ...
    console.log(datos);
    $http
      .post("http://143.110.233.84/masajes/addCitaMasaje", datos)
      .then(function () {
...
app.controller(
  ...
  function ($scope, $http, $location, $routeParams) {
    ...
    $http.get("http://143.110.233.84/masajes/ultimoExtra").then(function (res) {
      $scope.extraUltimo = res.data.extra[0].lastExtra;
      console.log($scope.extraUltimo);
    });

    $http
      .get("http://143.110.233.84/masajes/extras", {})
      .then(function (respuesta) {
        $scope.extras = respuesta.data.extras;
        var json_check = [];
        for (var i = 0; i < $scope.extras.length; i++) {
          var datosExtras = {
            id: $scope.extras[i].idExtras,
            nombre: $scope.extras[i].nombre,
            value: false,
            cantidad: 1,
          };
      ...
      if (
        ...
        for (var i = 0; i < $scope.checkboxModel.length; i++) {
          if ($scope.checkboxModel[i].value == true) {
            var datosExtras = {
              idExtra: $scope.checkboxModel[i].id,
              cantidad: $scope.checkboxModel[i].cantidad,
            };
    ...
    $scope.EnviarBD = function (data) {
      //console.log(data);
      $http
        .post("http://143.110.233.84/masajes/addExtrasMasaje", data)
        .then(function (respuesta) {
          console.log(respuesta.data);
          if (respuesta.data.status == 1) {
            $location.path("mas");
...
app.controller("reporteCtrl", function ($scope, $http) {
  ...
  consultarFechaDia = function (fecha) {
    ...
    $http
      .post("http://143.110.233.84/reportes/reporteVentasDia", data)
      .then(function (respuesta) {
        console.log(respuesta.data);
        $scope.total1 = 0;
  ...
  consultarConsultasDia = function (fecha) {
    ...
    $http
      .post("http://143.110.233.84/reportes/reporteConsultasDia", data)
      .then(function (respuesta) {
        $scope.totalCon;
  ...
  consultarConsultas = function (fecha, fecha2) {
    ...
    $http
      .post("http://143.110.233.84/reportes/reporteConsultas", data)
      .then(function (respuesta) {
        $scope.totalCon;
        $scope.extrasVentas = respuesta.data.consultas;
        let arrayAux = respuesta.data.consultas;
  ...
  consultarFecha = function (fecha, fecha2) {
    ...
    $http
      .post("http://143.110.233.84/reportes/reporteVentas", data)
      .then(function (respuesta) {
        console.log(respuesta.data);
        $scope.total1 = 0;
        let arrayAux = respuesta.data.ventas;
...
app.controller("ventasCtrl", function ($scope, $http, $location) {
  ...
  $http
    .get("http://143.110.233.84/inventario/allProducts", {})
    .then(function (respuesta) {
  ...
  $scope.aumentar = function (data) {
    ...
    for (var i = 0; i < $scope.lista.length; i++) {
      if (
        $scope.lista[i].idProducto == parseInt(data) &&
        $scope.lista[i].cantidad < existencia
      ) {
  ...
  $scope.Agregar = function (data) {
    var item = data.split("|");
    const producto = $scope.productos.find(
      (producto) => producto.idProducto == item[0]
    );
  ...
  $scope.Enviar = function (data) {
    if ($scope.lista != "") {
      ...
      $http
        .post("http://143.110.233.84/inventario/sellProduct", {
          products,
          total,
          cliente,
        })
  ...
  $scope.filtrarProducto = function () {
    for (var i = 0; i < $scope.lista.length; i++) {
      var objeto = {
        idProducto: parseInt($scope.lista[i].idProducto),
        cantidad: $scope.lista[i].cantidad,
      };
...
app.controller("altasCtrl", function ($scope, $http, $location) {
  ...
  $scope.Enviar = function (e, n, d, p) {
    ...
    console.log(data);
    $http
      .post("http://143.110.233.84/inventario/addProduct", data)
      .then(function (respuesta) {
        console.log(respuesta.data);
        if (respuesta.data.status == 1) {
          $scope.correcto = true;
...
app.controller("verproductosCtrl", function ($scope, $http, $location) {
  ...
  $scope.actualizar_existencia = function (id_pro, unidad_pro) {
    ...
    console.log(data);
    $http
      .post("http://143.110.233.84/inventario/updateProductInventory", data)
      .then(function (respuesta) {
        console.log(respuesta.data);
        if (respuesta.data.status == 1) {
          $scope.correcto = true;
